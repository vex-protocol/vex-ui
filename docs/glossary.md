...
| **Ed25519** | The signing scheme used for device identity in the default `tweetnacl` profile. The optional FIPS profile uses P-256 ECDSA instead | [vex-overview.md](vex-overview.md) |
...
| **nonce** | A fresh value used exactly once with an encryption key. The TweetNaCl profile uses 24-byte XSalsa20 nonces; the FIPS profile uses AES-GCM IVs of the required size | [vex-overview.md](vex-overview.md) |
...
| **signKey** | The public signing key of a device. It is Ed25519 in the default profile and P-256 ECDSA in the FIPS profile. Spire uses it to verify device proof and clients use it in session fingerprints | [vex-overview.md](vex-overview.md) |
